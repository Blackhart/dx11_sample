...
bool	D3DWrapper::GetDeviceInfo(uint16_t const pWidth, uint16_t const pHeight, uint32_t& pFreshRateNum, uint32_t& pFreshRateDenom, uint64_t& pVideoCardNameLength)
{
	HRESULT				lResult;
...
bool	D3DWrapper::InitializeRenderTarget()
{
	HRESULT				lResult;
...
bool	D3DWrapper::InitializeDepthBuffer(uint16_t const pWidth, uint16_t const pHeight)
{
	HRESULT							lResult;
...
bool	D3DWrapper::InitializeRasterizer()
{
	HRESULT					lResult;
...
void	D3DWrapper::InitializeViewport(uint16_t const pWidth, uint16_t const pHeight)
{
...
void	D3DWrapper::InitializeMatrix(uint16_t const pWidth, uint16_t const pHeight, float const pScreenFar, float const pScreenNear)
{
	float	lFieldOfView;
	float	lScreenAspect;
...
void	D3DWrapper::BeginScene(float const pRed, float const pGreen, float const pBlue, float const pAlpha)
{
	float	lClearColor[4];
...
ID3D11Device*	D3DWrapper::GetDevice() const
{
	return __device;
}

ID3D11DeviceContext*	D3DWrapper::GetDeviceContext() const
{
	return __deviceContext;
}

void	D3DWrapper::GetProjectionMatrix(XMMATRIX& pProjMatrix) const
{
	pProjMatrix = __projectionMatrix;
}

void	D3DWrapper::GetWorldMatrix(XMMATRIX& pWorldMatrix) const
{
	pWorldMatrix = __worldMatrix;
}

void	D3DWrapper::GetOrthoMatrix(XMMATRIX& pOrthoMatrix) const
{
	pOrthoMatrix = __orthoMatrix;
}

void	D3DWrapper::GetVideoCardInfo(char* pCardName, int& pMemory) const
{
